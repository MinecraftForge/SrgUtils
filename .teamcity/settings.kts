...
project {
    ...
    params {
        ...
        text("github_repository_name", "SrgUtils", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `SrgUtils` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "srgutils", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
...
object BuildSecondaryBranches : BuildType({
    ...
    id("SrgUtils__BuildSecondaryBranches")
